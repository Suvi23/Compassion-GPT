...
async function callAiEngine(messagesPayload, customSystemPrompt = null) {
  ...
  // Graceful Socratic fallback if network fails
...
window.handleUserMessage = async function(e, directQuery = null) {
  ...
  try {
    ...
    const msgContentId = 'msg-content-' + Date.now();
    chatContainer.insertAdjacentHTML('beforeend', `
      <div class="flex items-start space-x-2.5">
        ...
        <div class="chat-bubble-ai max-w-2xl rounded-2xl p-4 sm:p-5 text-xs sm:text-sm text-slate-800 space-y-2.5 shadow-xs">
          ...
          <div id="${msgContentId}" class="leading-relaxed space-y-2 text-slate-700"></div>
        </div>
      </div>
    `);

    // Stream words in real-time like ChatGPT
    const contentElem = document.getElementById(msgContentId);
...
How might adopting a daily intention of non-harm influence the small decisions you make throughout your day?`;
  ...
  // 2. DO PLANTS FEEL PAIN / PLANTS VS ANIMALS
  if (q.includes('plant') || q.includes('plants feel') || q.includes('do plants') || q.includes('tree') || q.includes('flora')) {
    return `### 🔬 Sentience vs. Signal Transduction: Do Plants Feel Pain?

A common question in comparative ethics asks whether plants experience suffering akin to animals. Modern plant neurobiology and animal physiology establish a fundamental distinction between biochemical signal transduction and centralized sentient experience.

**Key Scientific & Ethical Insights:**
• **Nociception & Central Nervous Systems:** Animals possess centralized brains, limbic structures, and specialized nociceptive C-fibers that produce valenced, felt subjective experiences of pain.
• **Evolutionary Function of Pain:** Pain evolved in motile organisms as an urgent behavioral signal to flee or avoid tissue damage. For sessile plants unable to move, valenced pain would offer no evolutionary utility; they rely instead on systemic chemical defenses.
• **Trophic Efficiency:** Even if one were concerned with plant impact, consuming plants directly spares far more plant life than feeding crops to livestock through inefficient trophic conversion (which requires 5–10x more plants).

**Practical Compassionate Actions:**
• Prioritize compassionate choices where empirical evidence of centralized consciousness and pain is strongest—in animals with nervous systems.
• Choose nutritious, protein-rich plant ingredients (lentils, chickpeas, nuts, grains, and vegetables).
...
**🤔 Socratic Reflection for Inquiry:**
How does understanding the biological difference between simple chemical reactions and felt subjective pain help clarify our moral responsibilities?`;
  }

  // 3. WILD ANIMAL SUFFERING / WELFARE BIOLOGY
  if ((q.includes('wild animal') || q.includes('wild wildlife') || q.includes('welfare biology') || q.includes('drought') || q.includes('winter freeze') || q.includes('predation')) && !q.includes('elephant') && !q.includes('deer')) {
    return `### 🌿 Wild Animal Suffering & Welfare Biology

For decades, popular narratives romanticized the natural world as an idyllic balance. However, contemporary welfare biology and suffering-focused ethics (Horta, 2010; Tomasik, 2015) recognize that wild animals endure immense natural suffering, including harsh winters, severe droughts, infectious diseases, and starvation.

**Key Scientific & Ethical Insights:**
• **The Naturalistic Fallacy:** Just because suffering occurs in nature does not make it desirable or ethically neutral. Pain feels just as real to a wild deer or songbird as to a companion pet.
• **High Juvenile Mortality:** In r-selected species, the vast majority of offspring perish shortly after birth from predation, exposure, or malnutrition.
• **Non-Invasive Stewardship:** Advances in veterinary medicine now permit non-invasive interventions, such as wildlife vaccination vectors and emergency drought water relief.

**Practical Compassionate Actions:**
• Support wildlife corridor conservation to allow free migration during climate droughts.
• Install anti-collision decals on reflective glass windows to prevent traumatic songbird deaths.
• Support research into humane, non-invasive wildlife health monitoring.

**📚 Recommended Academic References:**
• Horta, O. (2010). *Debunking the idyll of nature: Wild animal suffering*. Telos, 17(1), 73–89.
• Tomasik, B. (2015). *The Importance of Wild-Animal Suffering*. Relations: Beyond Anthropocentrism, 3(2), 133–152.

**🤔 Socratic Reflection for Inquiry:**
If suffering is intrinsically bad, does our moral circle encompass beings whose distress is caused by natural ecological conditions rather than human activity?`;
  }

  // 4. COMPASSION FATIGUE & SUSTAINABLE EMPATHY
...
When caring for a vulnerable dependent life, how does prioritizing their immediate physical comfort guide our next best decision?`;
  ...
  // 6. HORNETS / WASPS / BEES NEAR WINDOW DILEMMA
...
When deciding how to do good, why might preventing acute distress be more urgent than creating new luxuries?`;
  ...
  // 11. COGNITIVE BIASES / MEAT PARADOX
